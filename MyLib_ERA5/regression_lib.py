...
def standardize_series(arr, *, dim=None, ddof=1, return_stats=False):
    """Return a standardized (zero‑mean, unit‑variance) version of *arr*.

    Works with 1‑D/ND ``numpy.ndarray`` and ``xarray.DataArray``. By default the
    entire array is standardized. Use *dim* to specify the axis (for ``ndarray``)
    or dimension name(s) (for ``DataArray``) along which to compute the mean and
    standard deviation.
    ...
    dim : int, str or Sequence, optional
        Axis (``ndarray``) or dimension name(s) (``DataArray``) over which to
        compute statistics.  *None* means use the whole array.
    ddof : int, default 1
        Delta degrees‐of‐freedom used in the standard deviation divisor.
    return_stats : bool, default False
        If *True*, also return the mean and standard deviation used for
        standardization.

    Returns
    -------
    std_arr : same type as *arr*
        Standardized array.
    (mu, sigma) : tuple of arrays (optional)
        Mean and standard deviation, returned only when *return_stats* is True.
    """
    if isinstance(arr, xr.DataArray):
        mu = arr.mean(dim=dim, skipna=True)
        sigma = arr.std(dim=dim, ddof=ddof, skipna=True)
        std_arr = (arr - mu) / sigma
    else:
        arr = np.asarray(arr)
        mu = np.nanmean(arr, axis=dim)
        sigma = np.nanstd(arr, axis=dim, ddof=ddof)
        std_arr = (arr - mu) / sigma
    if return_stats:
        return std_arr, mu, sigma
    return std_arr

def compute_dmi(sst_da: xr.DataArray,
                *,
                west_bounds=(50, 70, -10, 10),
                east_bounds=(90, 110, -10, 0),
                lon_name="lon",
                lat_name="lat") -> xr.DataArray:
    """Compute the *Dipole Mode Index* (DMI) from SST.

    Parameters
    ----------
    sst_da : xarray.DataArray
        SST field with dimensions including *lon_name* and *lat_name*.
    west_bounds : tuple (lon1, lon2, lat1, lat2), default (50E–70E, 10S–10N)
        Box for the western pole (IOBM‐W).
    east_bounds : tuple (lon1, lon2, lat1, lat2), default (90E–110E, 10S–0)
        Box for the eastern pole (IOBM‐E).
    lon_name, lat_name : str
        Dimension names for longitude / latitude.

    Returns
    -------
    dmi : xarray.DataArray
        Time series of SST anomaly difference *west – east*.
    """
    # Helper to select sub‑box & area‑mean respecting 0‑360 wrap
    def _box_mean(da, bounds):
        lon1, lon2, lat1, lat2 = bounds
        lon = (da[lon_name] + 360) % 360  # ensure 0–360
        if lon1 < lon2:
            da_box = da.where((lon >= lon1) & (lon <= lon2), drop=True)
        else:  # box crosses 0° meridian
            da_box = da.where((lon >= lon1) | (lon <= lon2), drop=True)
        da_box = da_box.where((da[lat_name] >= lat1) & (da[lat_name] <= lat2), drop=True)
        return da_box.mean(dim=(lat_name, lon_name), skipna=True)

    west = _box_mean(sst_da, west_bounds)
    east = _box_mean(sst_da, east_bounds)

    dmi = west - east
    dmi.name = "DMI"
    dmi.attrs.update({
        "long_name": "Dipole Mode Index (west minus east SST)",
        "units": sst_da.attrs.get("units", "°C"),
        "west_box": str(west_bounds),
        "east_box": str(east_bounds),
    })
    return dmi
